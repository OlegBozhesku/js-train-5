...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (const index of restElements) {
    if (index < min) {
      min = index;
    } else if (index > max) {
      max = index;
...
function concatenateStrings(array, subString) {
  ...
  let result = test.includes(subString);
  console.log(result);
}
...
function replaceNumbers(arr) {
  ...
  for (const element of arr) {
    if (element > 10) {
      empty = [...empty, "Greater than 10"];
    } else if (element <= 10) {
      empty = [...empty, "Less than or equal to 10"];
    }
  }
  return empty;
}
